...
3. $E(\bar Y) = \mu$ and $Var(\bar Y) = \sigma^2/n$.  Here, $\mu$ is the mean of the distribution and $\sigma^2$ is the variance of the distribution (note these might be functions of other parameters governing the distribution; e.g. $\mu = \alpha\beta$ if the distribution is $GAM(\alpha,\beta)$). This is a result from Chapter 4.
4.  If $n$ is "large", then $\bar{Y} \sim N(\mu,\sigma^2/n)$.  This is a result of the **central limit theorem.**  We will prove this at the end of this handout.


**Facts when sample is drawn from a $N(\mu,\sigma^2)$ distribution:**

1.  **No matter what size $n$ is**,  $\bar{Y} \sim N(\mu,\sigma^2/n)$, where $\mu$ is the mean of the normal distribution and $\sigma^2$ is the variance of the normal distribution.
2. The following, useful for obtaining confidence intervals and doing hypothesis tests for $\sigma^2$ : $$\frac{(n-1)s^2}{\sigma^2} \sim \chi^2_{n-1}$$
3. The following,  useful for obtaining confidence intervals and doing hypothesis tests for $\mu$: $$\frac{\bar Y - \mu}{s/\sqrt{n}} \sim t_{n-1}$$
4. If $Y_1,Y_2,...,Y_n$ is an i.i.d. sample from a $N(\mu_Y,\sigma^2_Y)$ distribution; and $X_1,X_2,...,X_m$ is a sample drawn i.i.d from a $N(\mu_X, \sigma^2_X)$ distribution; then the ratio of the sample variances scaled as follows, $\frac{s^2_X/\sigma^2_X}{s^2_Y/\sigma^2_Y}$, follows an $F_{m-1,n-1}$ distribution. 

We will prove facts 1-4 when the sample (or samples, in the case of #4) is drawn from a normal population.  We will then prove the central limit theorem, which guarantees normality of $\bar Y$ for large $n$ when the sample comes from *any* distribution.
...
A.  If $Y\sim N(\mu,\sigma^2)$, then $Z = \frac{Y-\mu}{\sigma} \sim N(0,1)$ (Will prove.)
B.  If $Z \sim N(0,1)$, then $U = Z^2 \sim \chi^2_1\equiv GAM\left(\alpha=\frac{1}{2},\beta=2\right)$ (Proved using pdf method in Chapter 6). 
C.  If $Z_1,Z_2,...,Z_n$ are i.i.d. $\sim \chi^2_1$, then $\sum_{i=1}^n Z_i \sim \chi^2_n$.  (Will prove.)
...
**Using #2 to derive confidence intervals for $\sigma^2$**
...
**Using #2 for hypothesis testing**
